...
void configureGPIOInterrupt(uint8_t pin, uint8_t intr_type) {
    ...
    // Set interrupt type
    volatile uint32_t* int_type_reg = (volatile uint32_t*)(GPIO_PIN_INT_TYPE_REG + (reg_idx * 4));
    *int_type_reg |= (intr_type << (bit_pos * 2));
    
    // Enable interrupt for the pin
    volatile uint32_t* int_ena_reg = (volatile uint32_t*)(GPIO_PIN_INT_ENA_REG + (reg_idx * 4));
    *int_ena_reg |= (1 << bit_pos);
}
